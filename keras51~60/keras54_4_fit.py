...
hist = model.fit(xy_train,
                epochs=2,
                steps_per_epoch=32, # 전체 데이터/batch = 160/5 = 160/5 = 32
                validation_data=xy_test,
                validation_steps=24 # validation_data/batch = 120/5 = 24 
                )
...
# # ax2 = plt.subplot(2,2,4)   # 4x4의 두 번째
...
fig = plt.figure(figsize=(14, 5))
ax1 = plt.subplot(2,2,1)   # 4x4의 첫 번째
ax4 = plt.subplot(2,2,4)   # 4x4의 첫 번째
ax1.plot(acc, color='red')   # 첫 번째에 그림을 그린다.
ax1.plot(val_acc, color='blue')   # 첫 번째에 그림을 그린다.
ax4.plot(loss, color='black')   # 첫 번째에 그림을 그린다.
ax4.plot(val_loss, color='green')   # 첫 번째에 그림을 그린다.
# ax1.plot(acc, color='blue')   # 첫 번째에 그림을 추가한다.
plt.show()